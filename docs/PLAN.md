...
- **Bonus B resuelto: moderación de reseña individual sin banear al autor.** (2026-08-31.) `hideReview(id, reason)` / `showReview(id)` ocultan/restauran UNA reseña con `moderation_reason` + `hidden_by` (migración `20260831200000`), recalculando el promedio con el libro bloqueado; el autor queda activo y no se crean `BanAuditLog`. `moderationStatus` expone `moderation_reason` (fallback `ban_reason`). En la demo el admin tiene "Ocultar (moderación)" por reseña y "Mostrar (restaurar)" en el panel de ocultas. Ver `DECISIONES.md` → "Banear la cuenta vs. ocultar UNA reseña".
- **Bug resuelto: alternar ban/desban en la vista Moderación.** (2026-08-31.) El estado "ya baneado" se calculaba de una copia local del array de usuarios que nunca se refrescaba tras un ban/unban; ahora se consulta el estado real del usuario y la lista local se sincroniza. Re-banear o desbanear funciona sin recargar la vista.
- **Corrección: autores baneados también ven "Tu cuenta fue baneada".** (2026-08-31.) La card de la home y el Panel autor ahora se muestran para cualquier rol (no solo lectores) cuando el usuario está baneado, con motivo y apelación a soporte.
- **Corrección: reseña propia oculta bloquea el form de ese libro.** (2026-08-31.) El detalle de libro detecta la review oculta del usuario actual vía `moderationStatus.userId`, muestra la card "Tu reseña (oculta por moderación)" y reemplaza el form de crear por un box gris deshabilitado + aviso de contacto a soporte. Además `moderationStatus` pasó de scalar JSON a tipos GraphQL tipados (`ModerationStatusType`/`HiddenReviewType`), lo que además repara el error `Selections can't be made on scalars` que rompía la query contra el backend real.
